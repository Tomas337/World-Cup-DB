...
#echo "$($PSQL "SELECT o.name FROM (games INNER JOIN teams ON games.winner_id = teams.team_id) AS w INNER JOIN (games INNER JOIN teams ON games.opponent_id = teams.team_id) as o ON w.game_id = o.game_id")"
